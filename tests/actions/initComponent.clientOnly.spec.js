...
describe('initComponent with clientOnly action', () => {
  describe('when initMode === MODE_PREPARE', () => {
    describe('with { caller: \'willMount\' } option given', () => {
      ...
      it('does not call the action', () => new Promise((resolve) => {
        process.nextTick(() => {
    ...
    describe('with { caller: \'didMount\' } option given and init props', () => {
      it('calls the action with the correct values', () => {
        ...
        const MockWithInit = withInitAction(
          ['testInitProp'],
          { clientOnly: mockAction },
        )(SimpleInitTestComponent);

        store.dispatch(initComponent(
          MockWithInit,
          [45],
          'SimpleInitTestComponent[45]',
          { caller: 'didMount' },
        ));
        ...
        return new Promise((resolve) => {
          process.nextTick(() => {
            ...
            expect(mockAction.mock.results[0].value).resolves.toBe(45).then(resolve);
          });
        });
      });
      ...
      it('initially dispatches INIT_COMPONENT with { complete: false }', () => {
        clearComponentIds();
        const mockAction = jest.fn(({ testInitProp }) => new Promise(
          resolve => setTimeout(() => resolve(testInitProp), 40),
        ));
        const store = mockStore({ init: { mode: MODE_INIT_SELF, prepared: {} } });
        const MockWithInit = withInitAction(
          ['testInitProp'],
